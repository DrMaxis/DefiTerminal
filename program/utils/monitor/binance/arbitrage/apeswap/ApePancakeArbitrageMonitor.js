...
async function monitor(data) {
  ...
  web3.eth.subscribe('newBlockHeaders', (error, result) => {
    ...
    .on('data', async block => {
      ...
      const rawPancakeValue = await pancakeswap.router.methods.getAmountsOut(apeValueBN, [tokenOut, tokenIn]).call();
      ...
      console.log(pad(colors.red('tokenIn:'), 30), `${shiftedApeValue.toString()} ${tradingToken.name}`);

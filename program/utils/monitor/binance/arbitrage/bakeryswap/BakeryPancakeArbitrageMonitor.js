...
async function monitor(data) {
  ...
  web3.eth.subscribe('newBlockHeaders', (error, result) => {
    ...
    .on('data', async block => {
      ...
      console.log(pad(colors.yellow('Buying token at Bakeryswap DEX'), 30));
      ...
      const rawPancakeValue = await pancakeswap.router.methods.getAmountsOut(bakeryValueBN, [tokenOut, tokenIn]).call();
      const shiftedPancakeValue = await new BigNumber(rawPancakeValue[1]).shiftedBy(-tradingToken.decimals);
      const pancakeValueBN = await new BigNumber(rawPancakeValue[1]);

      console.log(pad(colors.yellow('Current I/O Values as of '), 30), moment().format('ll') + ' ' + moment().format('LTS'));
      console.log(pad(colors.yellow('Buying back token at Pancakeswap DEX'), 30));
      console.log(pad(colors.red('tokenIn:'), 30), `${shiftedBakeryValue.toString()} ${tradingToken.name}`);

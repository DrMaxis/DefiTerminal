...
async function fetchData(data) {
  ...
  web3.eth.subscribe('newBlockHeaders')
    .on('data', async block => {
      ...
      const currentEthPrice = (kyberRates.buy + kyberRates.sell) / 2;
      const profit1 = (parseInt(AMOUNT_TRADINGTOKEN_WEI) / 10 ** 18) * (kyberRates.sell - uniswapRates.buy)  * currentEthPrice;
      const profit2 = (parseInt(AMOUNT_TRADINGTOKEN_WEI) / 10 ** 18) * (uniswapRates.sell - kyberRates.buy)  * currentEthPrice;
      if(profit1 > 0) {
        console.log('Arb opportunity found!');
        console.log(`Buy ETH on Uniswap at ${kyberRates.buy} dai`);
        console.log(`Sell ETH on Kyber at ${uniswapRates.sell} dai`);
        console.log(`Expected profit: ${profit1} dai`);
        process.send({profit: profit1})
      } else if(profit2 > 0) {
        console.log('Arb opportunity found!');
        console.log(`Buy ETH from Kyber at ${uniswapRates.buy} dai`);
        console.log(`Sell ETH from Uniswap at ${kyberRates.sell} dai`);

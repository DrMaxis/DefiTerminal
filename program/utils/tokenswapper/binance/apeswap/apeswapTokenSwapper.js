...
const ApeTokenSwapContract = require('../../../../../build/contracts/ApeTokenSwap.json')
...
process.on('message', function (data) {
  ...
  async function swap(data) {
    ...
    const SwapContract = new web3.eth.Contract(ApeTokenSwapContract.abi, ApeTokenSwapContract.networks[networkID].address);
    ...
    let tokenPair = {
      address: await apeswap.factory.methods.getPair(fromToken.address, toToken.address).call()
    };
    ...
    const amountOutOfReturnToken = await apeswap.router.methods.getAmountsOut(swapAmountInWei, [fromToken.address, toToken.address]).call();
    ...
    const tx = await ApeTokenSwapContract.methods.startSwap(
      fromToken.address,
      toToken.address,
      swapAmountInWei,
      0,
